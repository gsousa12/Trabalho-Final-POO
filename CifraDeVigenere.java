...
public class CifraDeVigenere {
    ...
    private static int[][] createVigenereTable() {
        ...
        int[][] tableArr = new int[26][26]; // Matriz que vai receber a tabela de Vigenere

        for (int i = 0; i < 26; i++) { // Percorrer a Matriz

            for (int j = 0; j < 26; j++) { // Percorrer a Matriz

                int temp;

                if ((i + 65) + j > 90) { // Verifica se a soma de i + 65(representa a letra A na tabela ASCII) e j
                                         // ultrapassa o valor de Z na tabela ASCII(90). Se ultrapassar subtrai 26 para
                                         // manter dentro do intervalo de A a Z
    ...
    private static int itrCount(int key, int msg) {
        // this function will return the count which it takes from key's letter to reach
        // cipher letter
        // and then this count will be used to calculate decryption of encrypted letter
        // in message.
        int counter = 0;
        String result = "";
        for (int i = 0; i < 26; i++) {
    ...
    private static void cipherEncryption(String message, String mappedKey) {
        ...
        for (int i = 0; i < message.length(); i++) {
            if (message.charAt(i) == (char) 32 && mappedKey.charAt(i) == (char) 32) {
                ...
                // accessing element at table[i][j] position to replace it with letter in
                // message
